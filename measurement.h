...
void collectdata(const T1& val1, const T2& val2, const T3& val3, const std::string& fname){
    std::ofstream outfile(fname, std::ios::app);
    outfile << val1 << "," << val2 << "," << val3 << "\n";
    outfile.close();
}

template <typename T1, typename T2, typename T3, typename T4>
void collectdata(const T1& val1, const T2& val2, const T3& val3, const T4& val4, const std::string& fname) {
    std::ofstream outfile(fname, std::ios::app);
...
//measuring <S_L/2.Sj>
std::vector<Real> correlation_SoSi(MPS& psi, const SiteSet sites)
{
    auto N = length(psi);
    ...
    for (int i : range(N))
    {
         Corr[i] = 0.5*(spsm[Lo][i] + smsp[Lo][i]) + szsz[Lo][i];
    }
    return Corr;
}
...
{
    auto N = length(psi);
    //projecting MPS to a state with hole fixed at N/2
...
{
    auto N = length(psi);
    //project MPS to a state with hole fixed at N/2
    auto hole_op = sites.op("Cup*Cdagup*Cdn*Cdagdn",N/2);
    auto A = hole_op*psi(N/2);
    A.noPrime();
    psi.set(N/2,A);
...
void holeDynamics(MPS psi, const SiteSet sites, std::string fname)
{
    ...
    auto wf = sum(psi,phi,{"MaxDim", 500, "Cutoff", 1E-8});
    wf.position(N/2);
    wf /= norm(wf);
