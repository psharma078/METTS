...
collapse(MPS& phi,
    ...
    for(int j = 1; j <= N; ++j)
        ...
        printfln("probability = ", prob_em," ",prob_up, " ", prob_dn, \
                        " ", prob_ud," ",prob_up+prob_dn+prob_em+prob_ud);
 ...
 auto emState = ITensor(sj);
        ...
        if(direction == "Z")
            ...
            //upState.set(2,1.0);//interpret x as z basis to create a cps
            //dnState.set(3,1.0);//with definite Sz QN for SU(2) symm H
        ...
        //Set site j tensor
        phi.set(j,jstate);//site j is cps 1x1 matrix
        }
